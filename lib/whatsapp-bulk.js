...
function parseWhatsAppBulk(text) {
  ...
  function handleBody(body, explicitTs) {
    ...
    if (!parsed) {
      // Supports two-line entries like:
      // "cetaphil 14 de abril de 2026"
      // "R$ 143,98 cartao sergio"
      const pendingMatch = trimmed.match(
        /^(.+?)\s+\d{1,2}\s+de\s+[a-zà-ú]+\s+de\s+\d{4}$/i
      );
      if (pendingMatch && pendingMatch[1]) {
        pendingDescription = pendingMatch[1].trim();
        return;
      }
    ...
    if (pendingDescription && /^\s*R\$/i.test(trimmed)) {
